...
export default function PageHero({
  ...
  const renderH1 = () => {
    ...
    return (
      <>
        {h1.slice(0, idx)}
        <span className="text-[#b04a22]">{accentWord}</span>
  ...
  return (
    <section
      ...
      style={{
        background: "linear-gradient(135deg, #1c1a17 0%, #14120f 50%, #000000 100%)",
      }}
    >
      {/* Subtle red glow */}
      <div
        className="absolute top-0 left-1/4 w-96 h-96 rounded-full opacity-10 pointer-events-none"
        style={{ background: "radial-gradient(circle, #b04a22 0%, transparent 70%)" }}
      />
      ...
      <div className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {breadcrumbs.length > 0 && (
          ...
          >
            <Link href="/pg/home" className="text-[#7d8072] text-xs hover:text-[#b4b8a8] transition-colors">Home</Link>
            {breadcrumbs.map((crumb, i) => (
              <span key={i} className="flex items-center gap-1.5">
                <ChevronRight className="w-3 h-3 text-[#8a8d80]" />
                {crumb.href ? (
                  <Link href={crumb.href} className="text-[#7d8072] text-xs hover:text-[#b4b8a8] transition-colors">
                    {crumb.label}
                  </Link>
                ) : (
                  <span className="text-[#b4b8a8] text-xs">{crumb.label}</span>
                )}
              </span>
            ))}
          </motion.nav>
        ...
        <motion.h1
          ...
          className="text-3xl md:text-4xl lg:text-5xl font-bold text-[#b4b8a8] mb-4 max-w-3xl leading-tight"
        ...
        {subtext && (
          <motion.p
            ...
            className="text-[#a8ac9c] text-base md:text-lg max-w-2xl mb-8"
        ...
        {showCTA && (
          ...
          >
            ...
            <a
              ...
              style={{ border: "2px solid rgba(180,184,168,0.4)", color: "#b4b8a8" }}
